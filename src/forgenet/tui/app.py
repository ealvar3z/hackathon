"""Urwid-based TUI shell for local ForgeNet operations."""
...
class PageDefinition:
    """A navigable page in the ForgeNet operator shell."""
...
class ForgeNetTUIApplication:
    """Read-only operator shell over ForgeNet persistence."""
...
def run_tui(db_path: str | Path, cot_url: str) -> None:
    """Start the ForgeNet TUI for the given SQLite database."""

    create_all(db_path)
    session_factory = create_session_factory(db_path)
    ForgeNetTUIApplication(session_factory, cot_url).run()
