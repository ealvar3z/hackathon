"""Async PyTAK runtime wrapper for section4."""
...
class PyTAKRuntime:
    ...
    def __init__(self, cot_url: str, *, debug: bool = False) -> None:
        parser = ConfigParser()
        parser["section4"] = {"COT_URL": cot_url, "PYTAK_NO_HELLO": "1"}
        if debug:
            parser["section4"]["DEBUG"] = "1"

        self.config = parser["section4"]
    ...
    async def start(self) -> None:
        ...
        self._tasks = [
            asyncio.create_task(tx_worker.run(), name="section4-pytak-tx"),
            asyncio.create_task(rx_worker.run(), name="section4-pytak-rx"),
        ]
